...
class index {
    ...
    public function h5upload_bak() {
        ...
        if (!empty($_FILES)) {
            ...
            if ($_FILES["file"]["error"] || !is_uploaded_file($_FILES["file"]["tmp_name"])) {
                   error_log(print_r($_FILES["file"],1),3,'e:/1.txt');
    ...
    public function h5upload(){
        @set_time_limit(5 * 60);
        if (!$_FILES) {
            die('{"jsonrpc" : "2.0", "error" : {"code": 102, "message": "No files have been uploaded."}, "id" : "id"}');
        }

        $md5file = md5_file($_FILES['file']['tmp_name']);
        if($r = $this->db->get_one('attachment', array('md5file' => $md5file))) {
            // var_dump($md5file);die;
            $id = $r['id'];
            require_once(COREFRAME_ROOT.'app/core/libs/class/ImageOperation.class.php');
            $ImageOperation = new ImageOperation($r['path'],'original');
            $ImageOperation->image_init();
            die('{"jsonrpc" : "2.0", "exttype" : "img", "result" : "'.$r['path'].'", "id" : "'.$id.'", "filename" : "'.$r['name'].'" }');
        } else {
            $_username = get_cookie('bId_username');
            $wz_name = get_cookie('wz_name');
            $insert['username'] = $_username ? $_username : $wz_name;
            $clientIp = get_ip();
            /*if($insert['username']=='' && $clientIp!='123.126.20.110') {
                die('{"jsonrpc" : "2.0", "error" : {"code": 101, "message": "Not allow guest upload."}, "id" : "id"}');
            }*/
            require_once(COREFRAME_ROOT.'app/core/libs/class/ImgUpload.class.php');
            $ImgUploadModel = new ImgUpload();
            $info = $ImgUploadModel->upload($_FILES['file'],$GLOBALS['is_water'],$GLOBALS['is_allow_show_img']);
            if ($info) {
                
                $insert = array();
                $insert['name'] = $info['saveName'];
                $insert['path'] = $info['saveName'];
                $insert['addtime'] = time();
                $insert['filesize'] = $_FILES['file']['size'];
                $insert['ip'] = get_ip();
                $insert['md5file'] = $md5file;
                $attachment = load_class('attachment',M);
                $id = $attachment->insert($insert);
                require_once(COREFRAME_ROOT.'app/core/libs/class/ImageOperation.class.php');
                $ImageOperation = new ImageOperation($insert['name'],'original');
                $ImageOperation->image_init();
                die('{"jsonrpc" : "2.0", "exttype" : "img", "result" : "'.$info['saveName'].'", "id" : "'.$id.'", "filename" : "'.$info['saveName'].'" }');
            }else{
                die('{"jsonrpc" : "2.0", "error" : {"code": 103, "message": '.$ImgUploadModel->getError().'"}, "id" : "id"}');
            }
         ...
         public function upload_dialogorgin()
    {
        upload_url_safe();
        ...
        include T('attachment','upload_dialogorgin');
    }
