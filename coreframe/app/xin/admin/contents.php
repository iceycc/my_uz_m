...
require_once(COREFRAME_ROOT.'app/core/admin/phpqrcode.php');
/**
 * 内容添加
 */
load_class('admin');
...
class contents extends WUZHI_admin {
  public function listing(){
       ...
       include $this->template('contentm_listing');
     }  
        ...
        $addtime = empty($formdata['addtime']) ? SYS_TIME : strtotime($formdata['addtime']);
           ...
           var_dump($formdata['master_table']);
            ...
            if(isset($GLOBALS['submit'])) {
                MSG(L('add success'),'?m=xin&f=contents&v=listing&type='.$GLOBALS['type'].'&cid='.$cid.$this->su(),1000);
            } else {
            ...
            $show_formjs = 1;
            $show_dialog = 1;
            include $this->template('contentm_add');
        }
     }
  public function edit() {
      ...
      //插入时间，更新时间，如果用户设置了时间。则按照用户设置的时间
       $addtime = empty($formdata['addtime']) ? SYS_TIME : strtotime($formdata['addtime']);
            ...
            $show_formjs = 1;
            $show_dialog = 1;
            include $this->template('contentm_edit');
        }
     }
    public function de(){
         $db = load_class('db');
