...
<section class="wrapper">
    <div class="row">
            ...
            <section class="panel">
                <header class="panel-heading">
                 ...
                 <span class="dropdown examine">
                    ...
                    <form action="?m=xin&f=oldcontent&v=listing<?php echo $this->su();?>" class="form-inline"  method="post">
                ...
                &nbsp;&nbsp;&nbsp; &nbsp;&nbsp;<a href="?m=xin&f=content&v=listing&cid=39<?php echo $this->su();?>"class="btn btn-default btn-sm" >新活动</a>
                  <br/>

                   <font size="2px" style="margin-left:30px;">共<span style="color:red"><?php echo $total?></span>条记录.</font>
                <div class="panel-body" id="panel-bodys">
                <form action="?m=xin&f=oldcontent&v=Deletes<?php echo $this->su();?>" class="form-inline"  id="form6" method="post">
        ...
        <!--<a href="<?php if($Mjin['status']!=4) {echo $Mjin['url'];}else{ echo '?m=xin&f=oldcontent&v=Previews&id=<?php echo $Mjin["id"];?>'.$this->su();};?>" target="_blank"> -->
                             <? $str=R;$newstr = substr($str,0,strlen($str)-5);?>
                               ...
                               echo $newstr.'/mobile-template.html?temp='.$Mjin['id'];?><img class="xiaotu" src="<?echo $url?>" style="width:30px;" onClick="img_wei(this.src);"><a href="?m=xin&f=oldcontent&v=doDownload&filename=<?echo $url?><?php echo $this->su();?>">下载二维码</a></td>
                               <td style="padding-left:30px;"><?php 
                               if($Mjin['status']==1){echo "上线";}elseif($Mjin['status']==2){echo "下线";
                               }?></td>
                              <td> 
                              <a href="?m=xin&f=oldcontent&v=edit&id=<?php echo $Mjin['id'];?>&type=<?php echo $GLOBALS['type'];?>&cid=<?php echo $Mjin['cid'].$this->su();?>" class="btn btn-primary btn-xs">编辑</a>
                              <?php if($Mjin['status']==1){?><a href="javascript:makedo('?m=xin&f=oldcontent&v=UnShelve&id=<?php echo $Mjin['id'];?><?php echo $this->su();?>', '确认下线该记录？')" class="btn btn-default btn-xs">下线</a><?}elseif($Mjin['status']==2){?><a href="javascript:makedo(' ?m=xin&f=oldcontent&v=Putaway&id=<?php echo $Mjin['id'];?><?php echo $this->su();?>', '确认上线该记录？')" class="btn btn-default btn-xs">上线</a><?}elseif($Mjin['status']==3){?><a href="javascript:makedo('?m=xin&f=oldcontent&v=Publish&id=<?php echo $Mjin['id'];?><?php echo $this->su();?>', '确认发布该记录？')" class="btn btn-default btn-xs">发布</a><?}?>
                              <a href="javascript:makedo('?m=xin&f=oldcontent&v=delete&id=<?php echo $Mjin['id'];?><?php echo $this->su();?>', '确认删除该记录？')" class="btn btn-danger btn-xs">删除</a>
                                    </td>
                            <?php } ?>
                            </tbody>
                        </table>
...
<script type="text/javascript">
  $("select[name=key]").change(function(){
    var where = $(this).val();
    location.href="?m=xin&f=oldcontent&v=listing&shang="+where+"<?php echo $this->su();?>";
  })
  ...
  $("#deletes").click(function(){
    ...
    $("#bao1").click(function(){
    $("#form6").attr("action","?m=xin&f=oldcontent&v=listing<?php echo $this->su();?>")
  });
</script>
</body>
</html>