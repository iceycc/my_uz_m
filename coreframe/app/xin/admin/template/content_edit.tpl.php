...
<section class="wrapper">
    <div class="row">
        <div class="col-lg-12">
            ...
            <section class="panel">
              <form action="?m=xin&f=oldcontent&v=edit<?php echo $this->su();?>" class="form-inline"  id="form6" method="post">
              ...
              <div class="exercise">
                       ...
                       <?php
                         ...
                         <tr>
                            <td> <img src="<?php echo getMImgShow($vv['headpiece'],'original')?>"  alt="" onclick="img_viewss(this.src);" style="width:80px;margin-left: 20px;"/>
                                  ...
                                  <input type="text" name='bian[]' value="<?echo $vv['bian']?>" style="width:70px;height: 30px;margin-left: 90px;" placeholder="图片位置ID"><input type="hidden" name="type1[]"" value="img" >
                                  <a class="btn btn-danger btn-xs removeTr deleteButton" style="position:relative;left: 50spx;">删除</a></td>
                                  </tr>
                           <?}}?>
                     <tr>
                  ...
                  <tr id=''><!-- shipin -->
                    <th>
                      <font >视频页</font>:<?if($content['type']==1){?><span style="color:red" class='H5'>640*330</span><?}?>
                     <?if($content['type']==2){?> <span style="color:red" class='ISH5'>640*1136</span><br/><?}?>
                        <tr>
                        <?php 
                           $shipins = array();
                           $shipin = explode('|', $content['shipin']);
                           $bianp = explode('|', $content['shipinbian']);
                           $urlp = explode('|', $content['shipinurl']);
                           ...
                           foreach($shipins as $vvs){
                            if($vvs['shipin']!=""){?>
                            <td> <img src="<?php echo getMImgShow($vvs['shipin'],'original')?>"  alt="" onclick="img_viewss(this.src);" style="width:80px;margin-left: 20px;"/>
                                  <input type="text" name='shipin[]' value="<?echo $vvs['shipin']?>" style="width:300px;height: 30px;margin-left: 110px;">
                                  <input type="text" name="urlp[]" value="<?echo $vvs['urlp']?>" style="width:270px;height: 30px;margin-left: 90px;" placeholder="图片url连接(如:m.uzhuang.com)">
                                  <input type="text" name='bianp[]' id='picDes' value="<?echo $vvs['bianp']?>" style="width:70px;height: 30px;margin-left: 90px;" placeholder="图片位置ID"><input type="hidden" name="type2[]"" value="vim" >
                                  <a class="btn btn-danger btn-xs removeTr deleteButton" style="position:relative;left: 50spx;">删除</a></td>
                                  </tr>
                            <?}}?>
                       <tr>
                         <td>
                          <div class="attaclist"><div id="cases"><ul id="case_ul"></ul></div><span class="input-group-btn">
                           <button type="button" id="countLimit" class="btn btn-white" onclick="openiframe('/index.php?m=attachment&amp;f=index&amp;v=upload_dialog&amp;callback=callback_images10&amp;htmlid=cases&amp;limit=1&amp;width=1&amp;htmlname=form%5Bcases%5D&amp;ext=jpg%7Cpng%7Cgif%7Cbmp&amp;token=34f98e4c74f75c6701341b703f09917b&amp;_menuid=5002','cases','loading...',810,400,20)">上传图片</button>
                               </span>
                               <span>
                                 <?if($content['type']==1){?>
                                    <input type="radio" name="sx" value="1" <?php if($content['status_2']==1){echo 'checked';}?>>上&nbsp;&nbsp;&nbsp;
                                    <input type="radio" name="sx" value="2" <?php if($content['status_2']==2){echo 'checked';}?>>下</div>
                                    <?}?>
                                      <?if($content['type']==2){?>
                                      <input type="hidden" name='sx' value="1">
                                      <?}?>
                                      <font style="margin-left: 100px;position: relative;top: -21px;"><input type="checkbox" name="advertising" value="1" style="margin-left:5px;position: relative;top: 2px;" <?php if($content['advertising']==1){echo 'checked';}?>/>去广告(清谨慎勾选,点击是要花钱的)</font> 
                               </span>
                           </div>
                         </td>
                       </tr>  
            ...
            <button type="button" style="margin-left:550px;position:relative; margin-top:-30px" class="btn btn-white" onclick="openiframe('/index.php?m=attachment&amp;f=index&amp;v=upload_dialog&amp;callback=callback_thumb_dialog&amp;htmlid=attachment_test&amp;limit=1&amp;htmlname=setting%5Battachment_test%5D&amp;ext=png%7Cjpg%7Cgif%7Cdoc%7Cdocx&amp;token=3d9292f4c141b7f9c4f3a37f8af2e607&amp;_menuid=29&amp;_submenuid=67','attachment_test','loading...',810,400,1)">上传文件</button><br/><br/>
                       ...
                       <font >底部图片上传</font>: <span style="color:red">宽度640px  高度不限</span><br/>
                         ...
                         <tr>
                            <td> <img src="<?php echo getMImgShow($v['troops'],'original')?>"  alt="" onclick="img_viewss(this.src);" style="width:80px;margin-left: 20px;"/>
                                 ...
                                 <input type="text" name='bians[]' value="<?php echo $v['bians']?>" style="width:70px;height: 30px;margin-left: 90px;" placeholder="图片位置ID"><input type="hidden" name="type3[]" value="img" >
                   ...
                   <tr>
                    ...
                    <tr>
                       <th> <font  color='red'>是否显示报名区域</font>: 
                            <input type="radio" name="isno" class="is" value="1" <?php if($content['status_5']==1){echo 'checked';}?>>是&nbsp;&nbsp;&nbsp;
                            <input type="radio" name="isno" class="no" value="2" <?php if($content['status_5']==2){echo 'checked';}?>>否
                       </th>
                     </tr>
                     <input type="hidden" value='<? echo  $content['status_5']?>' id='isn'>
    ...
    if(v6){
     ...
     var res = reg.test(num);
        if(!res){
          alert("编号必须为数字");
          $(window).scrollTop(0);
          $("#picDes").css("border","1px solid red");
            return false;
        }
    }
   var v1 = $("#name").val();
...
$("#countLimit").mouseover(function(){
  ...
  var imgCount1 = parseInt($("#cases").closest("tr").prev("tr").find("td").length);
  var imgTotalCount = imgCount1+imgCount;
  if(imgTotalCount==1){
    $("#countLimit").attr("disabled","disabled");
  }
});
$("#cases").next("span").click(function(){
  ...
  if(d){
    alert("视频图片最多只能上传一张！！修改视频图片请删除现有图片在修改");
  }
}).mouseover(function(){
  var imgCount = parseInt($("#countLimit").closest("span").prev("#cases").find("#case_ul li").length);
  var imgCount1 = parseInt($("#cases").closest("tr").prev("tr").find("td").length);
  var imgTotalCount=imgCount1+imgCount;
  if(!imgTotalCount){
