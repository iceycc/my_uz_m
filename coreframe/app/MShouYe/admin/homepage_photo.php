...
load_class('admin');
define('HTML',true);
class homepage_photo extends WUZHI_admin {
    ...
    public function listing() {
        $keytypess = isset($GLOBALS['status'])&&!empty($GLOBALS['status']) ? $GLOBALS['status'] : 1;
        $page = isset($GLOBALS['page']) ? intval($GLOBALS['page']) : 1;
        $keytypes = isset($GLOBALS['keytypes'])&&!empty($GLOBALS['keytypes']) ? $GLOBALS['keytypes'] : '';
        $page = max($page,1);
        if(!empty($keytypess)||!empty($keytypes))
        {
            $where='status!=4';
               if($keytypess==1)
                {
              ...
              if(!empty($keytypes))
               {
                 $where.=" and city='".$keytypes."'";   
               }  

               if($GLOBALS['pai']=="tshang"){
                       $result = $this->db->get_list('tjw_picture',$where, '*', 0, 10,$page,'updatetime ASC');
                  }else if($GLOBALS['pai']=="txia"){
                       $result = $this->db->get_list('tjw_picture',$where, '*', 0, 10,$page,'updatetime DESC');
                  }else if($GLOBALS['pai']=="gshang"){
                       $result = $this->db->get_list('tjw_picture',$where, '*', 0, 10,$page,'intervene ASC');
                  }else if($GLOBALS['pai']=="gxia"){
                       $result = $this->db->get_list('tjw_picture',$where, '*', 0, 10,$page,'intervene DESC');
                  }else{
                       $result = $this->db->get_list('tjw_picture',$where, '*', 0, 10,$page,'updatetime DESC,sort DESC');
                  }
        }else{
                  if($GLOBALS['pai']=="tshang"){
                       $result = $this->db->get_list('tjw_picture',"status!='4'", '*', 0, 10,$page,'updatetime ASC');
                  }else if($GLOBALS['pai']=="txia"){
                       $result = $this->db->get_list('tjw_picture',"status!='4'", '*', 0, 10,$page,'updatetime DESC');
                  }else if($GLOBALS['pai']=="gshang"){
                       $result = $this->db->get_list('tjw_picture',"status!='4'", '*', 0, 10,$page,'intervene ASC');
                       // echo '<pre>';print_r($result);exit;
                  }else if($GLOBALS['pai']=="gxia"){
                       $result = $this->db->get_list('tjw_picture',"status!='4'", '*', 0, 10,$page,'intervene DESC');
                  }else{
                       $result = $this->db->get_list('tjw_picture',"status!='4'", '*', 0, 10,$page,'updatetime DESC,sort DESC');
                  }
        }
        $city=get_config('city_config');
        $pages = $this->db->pages;
        $total = $this->db->number;
        // var_dump($keytypes);die;
       include $this->template('homepage_photo_listing');
    }
    //添加案例
    public function add(){
        $cid ="270";
        ...
        if(isset($GLOBALS['submit']) || isset($GLOBALS['submit2'])) {
            ...
            if($GLOBALS['submit']=="发布"){
                     ...
                     $res = $this->db->get_list('tjw_picture','status!=4');
                     $titlearr=array();
                    ...
                    $formdata['cover'] = remove_xss(basename($GLOBALS['form']['cover']));
                    ...
                    $formdata['updatetime'] = empty($GLOBALS['form']['updatetime']) ? SYS_TIME : strtotime($GLOBALS['form']['updatetime']);
                    $formdata['status'] = '1';
                   
                   $id = $this->db->insert('tjw_picture',$formdata);
            ...
            if(isset($GLOBALS['submit'])) {
                if($GLOBALS['submit']=="存为草稿"){
                    MSG(L("存为草稿成功"),'?m=MShouYe&f=homepage_photo&v=listing&_su=wuzhicms');
                }
                  if($GLOBALS['submit']=="发布"){
                    MSG(L("发布成功"),'?m=MShouYe&f=homepage_photo&v=listing&_su=wuzhicms');
                }
            } else {
            ...
            include $this->template('homepage_photo_add');
        }
    }
    //编辑案例
    public function edit() {
        ...
        if(isset($GLOBALS['modelid']) && is_numeric($GLOBALS['modelid'])) {
         ...
         $re = $this->db->get_one('tjw_picture','id="'.$id.'"','*');
        ...
        if(isset($GLOBALS['submit']) || isset($GLOBALS['submit2'])) {
                  ...
                  if($GLOBALS['submit']=="发布"){
                    ...
                    $formdata['cid'] = 270;
                               $formdata['title'] = remove_xss($GLOBALS['title']);
                                ...
                                $formdata['cover'] = remove_xss(basename($GLOBALS['attachment_test']));
                                $formdata['updatetime'] = empty($GLOBALS['form']['updatetime']) ? SYS_TIME : strtotime($GLOBALS['form']['updatetime']);
                                $formdata['address'] = remove_xss($GLOBALS['address']);
                                $formdata['status'] = '1';
                
                    $this->db->update('tjw_picture',$formdata,array('id'=>$id));
                 }else if($GLOBALS['submit']=="存为草稿"){
                     //      $res = $this->db->get_list('tjw_picture','status!=4');
                     // $titlearr=array();
                     // foreach ($res as $key => $value) {
                     ...
                     // }
                             ...
                             $formdata['cid'] = 270;
                               $formdata['title'] = remove_xss($GLOBALS['title']);
                                $formdata['city'] = remove_xss($GLOBALS['city']);
                                $formdata['cover'] = remove_xss(basename($GLOBALS['cover']));
                                $formdata['updatetime'] = empty($GLOBALS['form']['updatetime']) ? SYS_TIME : strtotime($GLOBALS['form']['updatetime']);
                                $formdata['address'] = remove_xss($GLOBALS['address']);
                             $formdata['status'] = '3';
            ...
            MSG(L('案例编辑成功'),'?m=MShouYe&f=homepage_photo&v=listing&_su=wuzhicms');
        } else {
            ...
            if($model_r['attr_table']) {
                ...
                $attrdata = $this->db->get_one("tjw_picture",array('id'=>$id));
                $data = array_merge($data,$attrdata);
            ...
            $show_formjs = 1;
            $show_dialog = 1;
            include $this->template('homepage_photo_edit');
        }
    }
    public function intervene(){

       $result = $this->db->get_one('tjw_picture'," id='".$GLOBALS['id']."'");
       include $this->template('homepage_photo_intervene');
    }
    public function intervenes(){
        ...
        $this->db->update('tjw_picture',$formdata,array('id'=>$GLOBALS['id']));
