...
load_class('admin');
define('HTML',true);
class homepage_advertis extends WUZHI_admin {
    ...
    public function listing() {
        ...
        if(!empty($keytypess)||!empty($keytypes))
        {
            $where='status!=4';
              ...
              if(!empty($keytypes))
               {
                 $where.=" and city like '%,".$keytypes.",%'";   
               }  

               if($GLOBALS['pai']=="tshang"){
                       $result = $this->db->get_list('advertis',$where, '*', 0, 10,$page,'updatetime ASC');
                  }else if($GLOBALS['pai']=="txia"){
                       $result = $this->db->get_list('advertis',$where, '*', 0, 10,$page,'updatetime DESC');
                  }else if($GLOBALS['pai']=="gshang"){
                       $result = $this->db->get_list('advertis',$where, '*', 0, 10,$page,'intervene ASC');
                  }else if($GLOBALS['pai']=="gxia"){
                       $result = $this->db->get_list('advertis',$where, '*', 0, 10,$page,'intervene DESC');
                  }else{
                       $result = $this->db->get_list('advertis',$where, '*', 0, 10,$page,'updatetime DESC,sort DESC');
                  }
        }else{
                  if($GLOBALS['pai']=="tshang"){
                       $result = $this->db->get_list('advertis',"status!='4'", '*', 0, 10,$page,'updatetime ASC');
                  }else if($GLOBALS['pai']=="txia"){
                       $result = $this->db->get_list('advertis',"status!='4'", '*', 0, 10,$page,'updatetime DESC');
                  }else if($GLOBALS['pai']=="gshang"){
                       $result = $this->db->get_list('advertis',"status!='4'", '*', 0, 10,$page,'intervene ASC');
                       // echo '<pre>';print_r($result);exit;
                  }else if($GLOBALS['pai']=="gxia"){
                       $result = $this->db->get_list('advertis',"status!='4'", '*', 0, 10,$page,'intervene DESC');
                  }else{
                       $result = $this->db->get_list('advertis',"status!='4'", '*', 0, 10,$page,'updatetime DESC,sort DESC');
                  }
        }
       ...
       include $this->template('homepage_advertis_listing');
    }
    //添加案例
    public function add(){
        $cid ="270";
        ...
        if(isset($GLOBALS['submit']) || isset($GLOBALS['submit2'])) {
            ...
            if($GLOBALS['submit']=="发布"){
                     ...
                     $res = $this->db->get_list('advertis','status!=4');
                     $titlearr=array();
                   ...
                   $id = $this->db->insert('advertis',$formdata);
            ...
            if(isset($GLOBALS['submit'])) {
                if($GLOBALS['submit']=="存为草稿"){
                    MSG(L("存为草稿成功"),'?m=MShouYe&f=homepage_advertis&v=listing&_su=wuzhicms');
                }
                  if($GLOBALS['submit']=="发布"){
                    MSG(L("发布成功"),'?m=MShouYe&f=homepage_advertis&v=listing&_su=wuzhicms');
                }
            } else {
            ...
            include $this->template('homepage_advertis_add');
        }
    }
    //编辑案例
    public function edit() {
        ...
        if(isset($GLOBALS['modelid']) && is_numeric($GLOBALS['modelid'])) {
         ...
         $re = $this->db->get_one('advertis','id="'.$id.'"','*');
        ...
        if(isset($GLOBALS['submit']) || isset($GLOBALS['submit2'])) {
                  ...
                  if($GLOBALS['submit']=="发布"){
                    ...
                    $this->db->update('advertis',$formdata,array('id'=>$id));
                 }else if($GLOBALS['submit']=="存为草稿"){
                     //      $res = $this->db->get_list('advertis','status!=4');
                     // $titlearr=array();
                     // foreach ($res as $key => $value) {
            ...
            MSG(L('案例编辑成功'),'?m=MShouYe&f=homepage_advertis&v=listing&_su=wuzhicms');
        } else {
            ...
            if($model_r['attr_table']) {
                ...
                $attrdata = $this->db->get_one("advertis",array('id'=>$id));
                $data = array_merge($data,$attrdata);
            ...
            $show_formjs = 1;
            $show_dialog = 1;
            include $this->template('homepage_advertis_edit');
        }
    }
    public function intervene(){

       $result = $this->db->get_one('advertis'," id='".$GLOBALS['id']."'");
       include $this->template('homepage_advertis_intervene');
    }
    public function intervenes(){
        ...
        $this->db->update('advertis',$formdata,array('id'=>$GLOBALS['id']));
