...
load_class('admin');
define('HTML',true);
class homepage_special extends WUZHI_admin {
              ...
              if($keytypess==1){
               $where="status='1'";
                ...
                $where="status='3'";
                 ...
                 $result = $this->db->get_list('m_special',$where."and status!='4'", '*', 0, 10,$page,'updatetime DESC,sort DESC');
          }else if($keytypes){
                $where="title like  '%".$GLOBALS['title']."%'";
                $result = $this->db->get_list('m_special',$where."and status!='4'", '*', 0, 10,$page,'updatetime DESC,sort DESC');
          }else{
                if($GLOBALS['pai']=="tshang"){
                       $result = $this->db->get_list('m_special',"status!='4'", '*', 0, 10,$page,'updatetime ASC');
                  }else if($GLOBALS['pai']=="txia"){
                       $result = $this->db->get_list('m_special',"status!='4'", '*', 0, 10,$page,'updatetime DESC');
                  }else if($GLOBALS['pai']=="gshang"){
                       $result = $this->db->get_list('m_special',"status!='4'", '*', 0, 10,$page,'intervene ASC');
                  }else if($GLOBALS['pai']=="gxia"){
                       $result = $this->db->get_list('m_special',"status!='4'", '*', 0, 10,$page,'intervene DESC');
                  }else{

                       $result = $this->db->get_list('m_special',"status!='4'", '*', 0, 10,$page,'updatetime DESC,sort DESC');
                  }
              
          }
        $pages = $this->db->pages;
        $total = $this->db->number;
       include $this->template('homepage_special_listing');
    }
    ...
    public function add(){
        ...
        if(isset($GLOBALS['submit']) || isset($GLOBALS['submit2'])) {
            ...
            if($GLOBALS['submit']=="发布"){
                     ...
                     $res = $this->db->get_list('m_special','status!=4');
                     $titlearr=array();
                    ...
                    $formdata['special'] = remove_xss(basename($GLOBALS['form']['special']));
                   ...
                   $id = $this->db->insert('m_special',$formdata);
            ...
            if(isset($GLOBALS['submit'])) {
                if($GLOBALS['submit']=="存为草稿"){
                    MSG(L("存为草稿成功"),'?m=MShouYe&f=homepage_special&v=listing&_su=wuzhicms');
                }
                  if($GLOBALS['submit']=="发布"){
                    MSG(L("发布成功"),'?m=MShouYe&f=homepage_special&v=listing&_su=wuzhicms');
                }
            } else {
            ...
            include $this->template('homepage_special_add');
        }
    }//添加案例
    public function edit() {
        ...
        if(isset($GLOBALS['modelid']) && is_numeric($GLOBALS['modelid'])) {
         ...
         $re = $this->db->get_one('m_special','id="'.$id.'"','*');
        ...
        if(isset($GLOBALS['submit']) || isset($GLOBALS['submit2'])) {
                  ...
                  if($GLOBALS['submit']=="发布"){
                    ...
                    $formdata['special'] = remove_xss(basename($GLOBALS['attachment_test']));
                    $formdata['updatetime'] = empty($GLOBALS['form']['updatetime']) ? SYS_TIME : strtotime($GLOBALS['form']['updatetime']);
                    $formdata['address'] = remove_xss($GLOBALS['address']);
                    $formdata['status'] = '1';
                
                    $this->db->update('m_special',$formdata,array('id'=>$id));
                 }else if($GLOBALS['submit']=="存为草稿"){
                     //      $res = $this->db->get_list('m_special','status!=4');
                     // $titlearr=array();
                     // foreach ($res as $key => $value) {
            ...
            MSG(L('专题图编辑成功'),'?m=MShouYe&f=homepage_special&v=listing&_su=wuzhicms');
        } else {
            ...
            if($model_r['attr_table']) {
                ...
                $attrdata = $this->db->get_one("m_special",array('id'=>$id));
                $data = array_merge($data,$attrdata);
            ...
            $show_formjs = 1;
            $show_dialog = 1;
            include $this->template('homepage_special_edit');
        }
    }//编辑案例
    public function intervene(){
       $result = $this->db->get_one('m_special'," id='".$GLOBALS['id']."'");
       include $this->template('homepage_special_intervene');
    }
    public function intervenes(){
        ...
        $this->db->update('m_special',$formdata,array('id'=>$GLOBALS['id']));
