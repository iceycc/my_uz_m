...
class company extends WUZHI_admin {
   public function listing(){
        ...
        $page = max($page,1);
        $db = load_class('db');
        $c = $db->get_list('m_picture',"",'*');
        $cs = $db->number;
        $sc=$cs*200;
        $result = $db->get_list('m_picture'," status='1'",'*',0,$sc);//查询精
        $arr=array();//定义新的数组
      ...
      foreach ($pa as $key => $pas) {
        	if($pas['pa']==$GLOBALS['pai']){
        		if($pas['pan']=='shang'){
        			$company = $db->get_list('m_company',$where.'id IN('.implode(',',$arr).') ',"*", 0, 10, $page,$pas['su'].' ASC');
               foreach($company as $key=>$value){
                 $companyname= $db->get_one('linkage_data','lid='.$value['areaid_2'],'*');
                 $com= $db->get_one('company','id='.$value['id'],'*');
                 $company[$key]['com']=$companyname['name'];
               if($com['avg_total']=="0.0"){
                 ...
                 $db->update('m_company',$HONG,array('id'=>$value['id']));
        			...
        			$company = $db->get_list('m_company',$where.' id IN('.implode(',',$arr).')',"*", 0, 10, $page,$pas['su'].' DESC');
               foreach($company as $key=>$value){
               $companyname= $db->get_one('linkage_data','lid='.$value['areaid_2'],'*');
                $com= $db->get_one('company','id='.$value['id'],'*');
                 $company[$key]['com']=$companyname['name'];
