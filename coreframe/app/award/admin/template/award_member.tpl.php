...
<section class="wrapper">
    <div class="row">
            <section class="panel">
                    <form action="?m=award&f=award&v=member<?php echo $this->su();?>" class="form-inline"  method="post">
                         <select name="type" style="height:35px;">
                         <option value="mobile">手机号</option>
                         <option value="nickname">姓名</option>
                         <option value="icode">邀请码</option>
                         </select>
                          <input type="text" name="title" placeholder="手机号/邀请码" class="titles" style="height:35px; width:200px; " value="<?php echo $GLOBALS['title']?>" id='txtSearch'>
                    ...
                    <div class="layui-form">
                        <table class="layui-table">
                          <thead>
                            <tr>
                              <th style="padding: 9px 5px; text-align:center;">手机号</th>
                              <th style="padding:0px;text-align:center;">邀请码</th>
                              <th style="padding:0; text-align:center;">姓名</th>
                              <th style="padding: 9px 5px; text-align:center;">注册时间</th>
                              <th class="search" style="padding: 9px 5px; text-align:center;">
                              <select  name="key1">
                          ...
                          <?php foreach ($user as $key => $value) { ?>
                            <tr>
                              <td style="text-align:center;"><?php echo $value['mobile']; ?></td>
                              <td style="text-align:center;"><?php echo $value['icode']; ?></td>
                              <td style="text-align:center;"><?php echo $value['nickname']; ?></td>
                              <td style="text-align:center;"><?php echo $value['addtime']; ?></td>
                              <td class="shuxing" style="text-align:center;">
                              <select name="key">
                              <option value="0" <?php echo $value['attribute']==0?'selected':'' ?>>属性</option>
                              <option value='1' <?php echo $value['attribute']==1?'selected':'' ?>>社会人士</option>
                              <option value='2' <?php echo $value['attribute']==2?'selected':'' ?>>优装美家</option>
                              </select>
                              </td>
                            </tr>
                          <?php } ?>
...
layui.use('form', function(){
  ...
  $('.shuxing').on('mouseover',function(){
    $(this).find('dd').attr('onclick','change_opt(this)')
  });
  function change_opt(eleObj){
    var mobile= $(eleObj).closest('tr').find('td:eq(0)').text();
    var value = $(eleObj).attr('lay-value');
    $.post("?m=award&f=award&v=attribute&<?php echo $this->su();?>",{'value':value,'mobile':mobile},function(data){
    if(data.code==1){
      layer.msg('设置成功');
    }else{
      layer.msg('设置失败');
  ...
  function change_se(eleObj){
    ...
    location.href="?m=award&f=award&v=member&attribute="+attribute+"<?php echo $this->su();?>";
  }
