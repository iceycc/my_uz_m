			...
			<table class="table table-striped table-advance table-hover">
				<thead>
					<tr>
						...
						<th class="tablehead">注册来源</th>
						<th class="tablehead">注册时间</th>
						<th class="tablehead">管理操作</th>
					</tr>
				</thead>
				<tbody>
				<?php if(is_array($result))foreach($result as $r){ $i+=1;?>
					<tr id="u_<?php echo $r['uid'];?>">
						...
						<?php if($r['mobile_station']=='mobile'){ ?>
						<td>M站_手机号</td>
						<?php }else if($r['mobile_station']=='qq'){?>
						<td>M站_QQ</td>
						<?php }else if($r['mobile_station']=='weixin'){?>
						<td>M站_微信</td>
						<?php }else if($r['mobile_station']=='sina'){?>
						<td>M站_微博</td>
						<?php }else if($r['mobile_station']=='GAPP_mobile'){?>
						<td>官App_手机</td>
						<?php }else if($r['mobile_station']=='zhu_app_wx'){?>
						<td>官App_微信</td>
						<?php }else if($r['mobile_station']=='zhu_app_wb'){?>
						<td>官App_微博</td>
						<?php }else if($r['mobile_station']=='zhu_app_qq'){?>
						<td>官App_QQ</td>
						<?php }?>
						<td><?php echo date('Y-m-d', $r['regtime']);?></td>
						<td>
						   <a href="javascript:void(0)" onclick="view_user(<?php echo $r['uid'];?>)" class="btn btn-info btn-xs">详情</a>
...
function view_user(userid){
	top.openiframe('index.php?m=member&f=index&v=view&uid='+userid+'<?php echo $this->su();?>', 'view', '查看用户信息', 660, 560);
}
function edit(uid){
