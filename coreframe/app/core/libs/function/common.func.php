 ...
 * @return bool|string
 */
function time_format1($timestamp, $type = 0){
	if ($timestamp == 0) return '';
	$types = array('Y-m-d H:i:s', 'Y-m-d H:i', 'Y-m-d');
	$difftime = SYS_TIME - $timestamp;
	if ($difftime < 60) {
		return '刚刚更新';
	} else if($difftime < 3600 && $difftime >= 1) {
		$difftime = floor($difftime / 60);
		return $difftime . '分钟前更新';
	} else if($difftime<86400 && $difftime >= 3600){
	    $difftime = floor($difftime / 3600);
		return $difftime . '小时前更新';
	}else {
		return date($types[$type], $timestamp);
	}
}
...
function imagecut($imageurl, $width, $height, $flag = 1, $bgcolor = ''){
	...
	// return WEBURL.'image/'.$type.'/'.$imageurl;
	return getImgShow($imageurl , $type);
	/*if ($imageurl == '') return R . 'images/nopic.jpg';
	if (strpos($imageurl, PHOTOS_URL) === false) return R . 'images/nopic.jpg';;
	$attach_long = strlen(PHOTOS_URL);
	$tmpimageurl = substr($imageurl, $attach_long);
	if (!basename($tmpimageurl)){
		return R . 'images/nopic.jpg';
	} 
	$newimage = dirname($tmpimageurl) . '/img_' . $width . '_' . $height . '_' . basename($tmpimageurl);
	if (file_exists(PHOTOS_ROOT . $newimage)) return PHOTOS_URL . $newimage;

	if (!file_exists(str_replace(PHOTOS_URL, PHOTOS_ROOT, $imageurl))) return $imageurl;
	$image = load_class('image');
	if ($image->set_image(PHOTOS_ROOT . $tmpimageurl) === false) return R . 'images/nopic.jpg';
	$image->createImageFromFile();
	if ($bgcolor) {
		$bgcolor = explode(',', $bgcolor);
	}
	$image->resizeImage($width, $height, $flag, $bgcolor);
	$image->save(1, PHOTOS_ROOT, $newimage);
	$image->save(1, PHOTOS_ROOT, $newimage);
	return PHOTOS_URL . $newimage;*/
}

//返回图片的链接地址
function getImgShow($src,$type){
	$srv_n = (crc32($src) % 5) +1;
	return 'http://image'.$srv_n.'.uzhuang.com/image/'.$type.'/'.$src;
}

function getMImgShow($src,$type){
	  $str=R;$newstr = substr($str,0,strlen($str)-5);
      $newstrs=trim($newstr,"http://");
       if($newstrs=="m.uzhuang.com"){
       	$srv_n = (crc32($src) % 2) +1;
        return 'http://mimage'.$srv_n.'.uzhuang.com/image/'.$type.'/'.$src;
       }else if($newstrs=="mdev.uz.com"){
         return 'http://m0.uz.com/image/'.$type.'/'.$src;
       }else if($newstrs=="mtest.uz.com"){
         return 'http://m1.uz.com/image/'.$type.'/'.$src;
       }else if($newstrs=="mpre.uz.com"){
        return 'http://m2.uz.com/image/'.$type.'/'.$src;
       }
}
function getGImgShow($src,$type){
	  $str=R;$newstr = substr($str,0,strlen($str)-5);
      $newstrs=trim($newstr,"http://");
      return $newstr.'/'.'image/'.$type.'/'.$src;
}
function getImgShownew($src,$type){
	$srv_n = (crc32($src) % 5) +1;

	return 'http://image-'.$srv_n.'.uzhuang.com/uploadfile/'.substr($src,0,2).'/'.substr($src,2,2).'/'.$src;
}
function getImgShownew1($src,$type){
 
     $s=$src['url'];
     var_dump($s);die;
	$srv_n = (crc32($src) % 5) +1;
	return 'http://image-'.$srv_n.'.uzhuang.com/uploadfile/'.substr($src,0,2).'/'.substr($src,2,2).'/'.$src['url'];
}
 ...
 * @param $domain
 */
function shopurl($uid,$domain = '',$subdomain = 0,$cityid = 0) {
	static $city_config;
	if(empty($city_config)) {
		$city_configs = get_config('city_config');
		$city_config = array();
		foreach($city_configs as $key=>$r) {
			$city_config[$r['cityid']] = $key;
		}
	}
	if($subdomain) {
		return 'http://' . $domain . '.uzhuang.com/';
	}elseif($domain) {
		return WEBURL.$city_config[$cityid].'-zhuangxiugongsi/'.$domain.'/';
	} else {
		return WEBURL.$city_config[$cityid].'-zhuangxiugongsi/'.$uid.'/';
	}
}
...
function filterurl($_type,$_field,$_value) {
	...
	switch($_type) {
		case 'photo'://单图
			...
			$urlrule = '/shejitu/{$style}-{$space}-{$color}-{$order}-1.html';
			$variables = array();
		...
		case 'photos'://图集
			...
			$urlrule = '/xiaoguotu/{$style}-{$house}-{$area}-{$cost}-{$color}-{$order}-1.html';
			$variables = array();
			foreach($field_array as $field) {
				$variables[$field] = isset($GLOBALS[$field]) ? intval($GLOBALS[$field]) : 0;
			}
			...
			break;
		case 'company'://装修公司
			$field_array = array('city','areaids','typeids','style','tese','order');
			$urlrule = '/{$city}-zhuangxiugongsi/{$areaids}-{$typeids}-{$style}-{$tese}-{$order}-1/';
		...
		case 'design'://设计师
			...
			$urlrule = '/shejishi/{$style}-{$years}-{$money_type}-{$order}-1.html';
			...
			break;
			case 'jialog'://管家日志
			$field_array = array('style','house','area','cost','order');
			$urlrule = '/jialog/{$style}-{$house}-{$area}-{$cost}-{$order}-1.html';
 ...
 */
function check_sms_code($smscode, $mobile)
{
    $db = load_class('db');
    $res = $db->get_one('sms_checkcode', "mobile = {$mobile}", '*', 0, 'id DESC');
    if(!$res || $res['code']=='' || $res['code']!=$smscode){
        return false;
    }
    return true;
}
