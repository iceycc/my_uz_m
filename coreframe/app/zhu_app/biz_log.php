...
defined('IN_WZ') or exit('No direct script access allowed');
    ...
    public function bag(){
        ...
        $arr=array();
        if($acquiesce==0){
        $log_rss = $this->db->get_list('day_log_demand_list',"addtime<'2016-07-01 11:30:16' and orderid!=3375 and orderid!=1550", 'logname,userid,recphoto,addtime,nodename,orderid,nodeid,uid');
        $log_rs = $this->db->get_list('day_log_demand_list',"addtime<'2016-07-01 11:30:16' and orderid!=3375 and orderid!=1550", 'logname,userid,recphoto,addtime,nodename,uid,orderid,nodeid', 0, 10, $page, 'addtime DESC');
        $c=count($log_rss);
        $page_max=ceil($c/10);
        }
        //var_dump($arr['c1']);exit;
        if($acquiesce==1) {
        $log_rsss = $this->db->get_list('day_log_demand_list',"addtime<'2016-07-01 11:30:16' and orderid!=3375 and orderid!=1550", 'logname,userid,recphoto,addtime,nodename,orderid,nodeid,uid');
        $log_rs = $this->db->get_list('day_log_demand_list',"addtime<'2016-07-01 11:30:16' and orderid!=3375 and orderid!=1550", 'logname,userid,recphoto,addtime,nodename,orderid,nodeid,uid', 0, 10, $page, 'browse_count DESC');
        $c=count($log_rsss);
        $page_max=ceil($c/10);
        }
        if($nodeid){  
        $where = "addtime<'2016-07-01 11:30:16' and orderid!=3375 and orderid!=1550 and nodeid in (".$day_log[$nodeid]['nodeid'].")";
        $log_rssss = $this->db->get_list('day_log_demand_list',$where, 'logname,userid,recphoto,addtime,nodename,orderid,nodeid,uid');
        $log_rs = $this->db->get_list('day_log_demand_list',$where, 'logname,userid,recphoto,addtime,nodename,orderid,nodeid,uid', 0, 10, $page, 'addtime DESC');
        $c=count($log_rssss);
        $page_max=ceil($c/10);
        ...
        if($ends==5){
          //echo "string";
        $where = "addtime<'2016-07-01 11:30:16' and orderid!=3375 and orderid!=1550 and orderstatus='完结'";
    ...
    public function log(){
          ...
          //var_dump($retest);exit;
           ...
           if($uid){
           $collectstatus1=$this->db->get_one('favorite',"`keyid`=$orderid and `uid`=$uid and `type`=13",'keyid,collectstatus');
                 if(!$collectstatus1){
                    $arr['collectstatus']='0';
                 }else{
              ...
              $arr['nodeid']=$maxNodeId[0]['nodeid']; 
              if($photoInfo){
              $arr['maxNodeId']['recphoto'] = $photoInfo;
              }else{
              $arr['maxNodeId']['recphoto'] =''; 
              }
              if($photoInfos){
              $arr['maxNodeId']['sitephoto'] = $photoInfos;
              }else{
              $arr['maxNodeId']['sitephoto']=''; 
              }
