...
class zxkt_index extends WUZHI_foreground {
  ...
  public function video_lecture(){
    $res=$this->db->get_list('faq',"cid=290 and status=9",'title,remark,url,thumb',0,2,$page,'addtime desc');
  ...
  public function video_list(){
    ...
    $time=$this->db->get_list('faq',"cid=290 and status=9",'addtime',0,1,$page,'addtime desc');
    //echo "<pre>";print_r($time);exit;
    $addtime=$time[0]['addtime'];
    if($time[0]['addtime']==$etag){
      header("HTTP/1.1 304 Not Modified");
      //echo "string";
    }else{
    $ress=$this->db->get_list('faq',"cid=290 and status=9",'title,remark,url,thumb');
    $res=$this->db->get_list('faq',"cid=290 and status=9",'title,remark,url,thumb',0,10,$page,'sort desc,addtime desc');
    $c=count($ress);
    $page_max=ceil($c/10);
    foreach ($res as $key => $value) {
      if($value['thumb']){
    ...
    foreach ($res as $key => $value) {
      ...
      /*$collectstatus=$this->db->get_list('favorite',$where11,'keyid,collectstatus');
        $ar=array();
            ...
            if(empty($uid)){
              $res[$key]['collectstatus'] = (empty($uid)) ? $arr['collectstatus']=0:$arr['collectstatus']=$collectstatus1['collectstatus'];
            }else{
            if(in_array($value['cid'],$ar)){
  ...
  public function owner_story_list(){
    ...
    $ress=$this->db->get_list('faq',"cid=300 and status=9",'id,url,title,remark,thumb,addtime',0,100,$page,'sort desc,addtime desc');
    //$ress=$this->db->get_list('faq',"cid=293 and status=9",'id,url,title,remark,thumb,addtime',0,100,$page,'sort desc,addtime desc');
    foreach ($ress as $keys => $values) {
      $ress[$keys]['video_url']=$values['url'];
      $ress[$keys]['addtime']=date('Y-m-d',$values['addtime']);
      if($value['thumb']){
          $ress[$keys]['thumb']="http://www.uzhuang.com/image/pic_230/".$values['thumb'];
      }
    }
    $arr=array(
      'video'=>$ress,
  ...
  public function chapter(){
    ...
    public function detail(){
      ...
      if($id){
        ...
        $faq = $this->db->get_one('faq', array('id' => $id),"browse");
        $browseCount = (int)$faq['browse']+1;
        $this->db->update('faq',array('browse'=>$browseCount),array('id'=>$id));
        if(!empty($res)){
        echo json_encode(array('code'=>1,'data'=>$res,'message'=>'操作成功','process_time'=>time())); exit;
        }else{
    ...
    /*public function collection(){
       ...
       $collectstatus=$this->db->get_list('favorite',"type=16",'keyid,collectstatus');
           ...
           $ar=array();
              ...
              echo json_encode(array('code'=>0,'data'=>$arrs,'message'=>'失败','process_time'=>time())); exit;
              }else{
                if(in_array($id,$ar)){
                   $a=$this->db->get_one('favorite',"`keyid`='".$id."' and `uid`='".$uid."'",'collectstatus');
                   ...
                   if(empty($a)){
                    ...
                    $arr['collectstatus']=$a['collectstatus'];
                     $arrs=array_values($arr);
                    echo json_encode(array('code'=>1,'data'=>$arrs,'message'=>'成功','process_time'=>time())); exit;
                   }
                }else{
                  $arr['collectstatus']='0';
                   $arrs=array_values($arr);
