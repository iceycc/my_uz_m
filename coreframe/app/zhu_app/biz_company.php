...
public function company_details(){
        // $uid=get_cookie('_uid');
        $uid=$GLOBALS['uid'];
        $companyid = empty($GLOBALS['companyid'])?'':mysql_escape_string($GLOBALS['companyid']);
     ...
     if($uid){
        //取收藏状态
            ...
            if(in_array($companyid,$arr)){
                 $collectstatus1=$this->db->get_one('favorite',"`keyid`=$companyid and `uid`=$uid and `type`=12",'keyid,collectstatus');
            }
            if(!$collectstatus1['collectstatus']){
              $collectstatus1['collectstatus']=(int)0;
            }
         //登录状态
        $login_status=(int)1;
