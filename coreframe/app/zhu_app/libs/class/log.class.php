...
class WUZHI_log extends WUZHI_foreground {
    ...
    public function result0(){
        ...
        $company= $this->db->get_list('demand_track_detail',"orderid='".$order_id."' and nodeid=10",'col2,addtime');
        ...
        foreach ($arr as $key => $value) {
         ...
         foreach ($a as $kes => $vals) {
          if(!empty($vals)){
           $photos[]="http://www.uzhuang.com/image/pic_230/".$vals;
          }
         }
        }
        ...
        $result['photo']=array_slice(array_values(array_filter(array_merge($recphoto,$photos))),0,9);
    ...
    public function result1(){
        ...
        $result['nodename']=$res['nodename'];
        $company= $this->db->get_list('demand_track_detail',"orderid='".$order_id."' and nodeid=11 and col8='是'",'col2,col8');
        foreach ($company as $ke => $va) {
           //echo "<pre>";print_r($va['col8']);
               $result['companyname'][]=$va;
        }
        $resu['check_other']=string2array($res['check_other']);
        $arr=array();
    ...
    public function result18(){
        ...
        $resu=  $this->db->get_one('demand_track',array('orderid'=>$order_id,'nodeid'=>37),"addtime");
        $date1=strtotime($resu['addtime']);
        $result['addtime1']= date('Y-m-d',$date1);
        $result['nodename']=$res['nodename'];
        //$pinglun =$this->db->get_list('log_evaluate',"orderid=2295 and nodeid=17",'*',0,1000,$page,'id DESC');
        $resu['check_other']=string2array($res['check_other']);
        $arr=array();
        ...
        //echo "<pre>";print_r($result['gjphoto']);
    ...
    public function pays($order_id,$nodeid){
        ...
        $sort[]=array_unique($deman);
            $arr[]=$this->db->get_list('demand_referno',"orderid='".$order_id."' and nodeid = '".$nodeid."'",'nodename,nodeid,needmoney,extrapay',0,100,$page,'');
