...
class zhu_index_new extends WUZHI_foreground {
...
/*    //工地推荐
    ...
    //灵感图库
    public function case_photo($city,$style,$uid,$style1){
      $array=array();
        if($style){
          if(!empty($style1)){
            ...
            $pieces = implode(",",$id); 
             $picture= $this->db->get_list('m_picture',"companyid in (".$pieces.") and style='".$style."' and status=1","id,cover,name,area,total,style,designer,addtime",0,9,$page,'intervene desc,updatetime desc');
             $coun=count($picture);
             $sta = (9-$coun);
             if($sta!=0){
             $pictures= $this->db->get_list('m_picture',"companyid in (".$pieces.") and style='".$style1."' and status=1","id,cover,name,area,total,style,designer,addtime",0,$sta,$page,'intervene desc,updatetime desc');
             }
             //echo "<pre>";print_r($sta);exit;
             /*if($coun>=2){
              $pictures= $this->db->get_list('m_picture',"companyid in (".$pieces.") and style='".$style1."' and status=1","id,cover,name,area,total,style,designer,addtime",0,1,$page,'intervene desc,updatetime desc');
             }else if($coun==1){
              $pictures= $this->db->get_list('m_picture',"companyid in (".$pieces.") and style='".$style1."' and status=1","id,cover,name,area,total,style,designer,addtime",0,2,$page,'addtime desc');
             }else if($coun==0){

              $pictures= $this->db->get_list('m_picture',"companyid in (".$pieces.") and style='".$style1."' and status=1","id,cover,name,area,total,style,designer,addtime",0,3,$page,'intervene desc,updatetime desc');
             }*/
              if($picture){
          ...
          $pieces = implode(",",$id);
          $picture= $this->db->get_list('m_picture',"companyid in (".$pieces.") and style='".$res."' and status=1","id,cover,name,style,area,total,designer,addtime",0,9,$page,'intervene desc,updatetime desc');
          foreach ($picture as $kk => $vv) {
              $a[]=$vv;  
          }
        }
        ...
        foreach ($a as $keys => $values) {
          ...
          //http://m.uzhuang.com/image/original/8f3f0020l40e2000o8ovy5.jpg
          $picture[$keys]['cover']='http://m.uzhuang.com/image/original/'.$values['cover'];
          ...
          $picture[$keys]['area']=$values['area'];
          $picture[$keys]['total']=$values['total'];
          $picture[$keys]['addtime']=$values['addtime'];
        } 
      if($picture){
      return $picture;
      }
    ...
    //推荐装修攻略
    public function getOrder($uid,$status,$set_node){
        $nodes = $this->getNode();
        ...
        if($res){
            ...
            $return[] = $this->nodeReflection('',$xnode['node_id'],$set_node);
        }else{
          if(empty($return)){
              $return[] = $this->nodeReflection($status,'',$set_node);
          }
        }
        $nodes = $this->getNode();
        return $return;
    ...
    public function nodeReflection($status, $node=0,$set_node){
        $args = array();
        $flag = false;
        if(empty($status) && empty($node)){
            ...
            $ress=$this->db->get_one('faq',$where,'id,title,remark,thumb,url,updatetime,browse',0,1,$page,'addtime desc');
            if($ress){
                $ress['thumb'] = $ress['thumb']?"http://www.uzhuang.com/image/pic_230/".$ress['thumb']:'';
                $data['arr'][] = $ress;
                $data['total']--;
                foreach($this->db->get_list('faq',"cid=70 and status=9",'id,title,remark,thumb,updatetime,browse',0,2,$page,'addtime desc') as $item){
                ...
                foreach($this->db->get_list('faq',"cid=71 and status=9",'id,title,remark,thumb,updatetime,browse',0,$data['total'],$page,'addtime desc') as $item){
                    $item['thumb'] = $item['thumb']?"http://www.uzhuang.com/image/pic_230/".$item['thumb']:'';
                    if(WEBURL=='http://m.uzhuang.com/'){
                    $item['urls']="http://m.uzhuang.com/mobile-uzapp_article.html?uzid=".$item['id'];
                    }else if(WEBURL=='http://mdev.uz.com/'){
                ...
                foreach($this->db->get_list('faq',"cid=70 and status=9",'id,title,remark,thumb,updatetime,browse',0,3,$page,'addtime desc') as $item){
                    $item['thumb'] = $item['thumb']?"http://www.uzhuang.com/image/pic_230/".$item['thumb']:'';
                    if(WEBURL=='http://m.uzhuang.com/'){
                    $item['urls']="http://m.uzhuang.com/mobile-uzapp_article.html?uzid=".$item['id'];
                    }else if(WEBURL=='http://mdev.uz.com/'){
            ...
            if($ress){
                ...
                $data['arr'][] = $ress;
                $data['total']--;
                foreach($this->db->get_list('faq',"cid=73 and status=9",'id,title,remark,thumb,updatetime,browse',0,2,$page,'addtime desc') as $item){
                    $item['thumb'] = $item['thumb']?"http://www.uzhuang.com/image/pic_230/".$item['thumb']:'';
                    if(WEBURL=='http://m.uzhuang.com/'){
                    $item['urls']="http://m.uzhuang.com/mobile-uzapp_article.html?uzid=".$item['id'];
                    }else if(WEBURL=='http://mdev.uz.com/'){
                ...
                foreach($this->db->get_list('faq',"cid=74 and status=9",'id,title,remark,thumb,updatetime,browse',0,$data['total'],$page,'addtime desc') as $item){
                    $item['thumb'] = $item['thumb']?"http://www.uzhuang.com/image/pic_230/".$item['thumb']:'';
                    if(WEBURL=='http://m.uzhuang.com/'){
                    $item['urls']="http://m.uzhuang.com/mobile-uzapp_article.html?uzid=".$item['id'];
                    }else if(WEBURL=='http://mdev.uz.com/'){
                ...
                foreach($this->db->get_list('faq',"cid=73 and status=9",'id,title,remark,thumb,updatetime,browse',0,3,$page,'addtime desc') as $item){
                    $item['thumb'] = $item['thumb']?"http://www.uzhuang.com/image/pic_230/".$item['thumb']:'';
                    $data['arr'][] = $item;
                    $data['total']--;
                }
                foreach($this->db->get_list('faq',"cid=75 and status=9",'id,title,remark,thumb,updatetime,browse',0,$data['total'],$page,'addtime desc') as $item){
                    $item['thumb'] = $item['thumb']?"http://www.uzhuang.com/image/pic_230/".$item['thumb']:'';
                    $data['arr'][] = $item;
                }
            }
        ...
        foreach($args as $key => $value){
            ...
            for($i=0; $i<$_count['num']; $i++){
                ...
                $temp[] = $offset;
                foreach ($this->db->get_list('faq',$value['where'],'id,title,remark,thumb,updatetime,browse',$offset,1,1,'addtime desc') as $item) {
                    $item['thumb'] = $item['thumb']?"http://www.uzhuang.com/image/pic_230/".$item['thumb']:'';
                    if(WEBURL=='http://m.uzhuang.com/'){
                    $item['urls']="http://m.uzhuang.com/mobile-uzapp_article.html?uzid=".$item['id'];
                    }else if(WEBURL=='http://mdev.uz.com/'){
                    ...
                    $item['updatetime'] = time_format1($item['updatetime']);
                    if(!preg_match("/[\x7f-\xff]/", strtotime($item['updatetime']))){
                ...
                if($value['limit']==1){
                  foreach ($this->db->get_list('faq',$value['where'],'id,title,remark,thumb,url,updatetime,browse',0,$value['limit'],1,'addtime desc') as $item) {
                      $item['thumb'] = $item['thumb']?"http://www.uzhuang.com/image/pic_230/".$item['thumb']:'';
                      $item['updatetime'] = time_format1($item['updatetime']);
                      if(!preg_match("/[\x7f-\xff]/", strtotime($item['updatetime']))){
                        $item['updatetime']=date('Y-m-d',strtotime($item['updatetime']));
                  ...
                  foreach ($this->db->get_list('faq',$value['where'],'id,title,remark,thumb,updatetime,browse',0,$value['limit'],1,'addtime desc') as $item) {
                    $item['thumb'] = $item['thumb']?"http://www.uzhuang.com/image/pic_230/".$item['thumb']:'';
                    if(WEBURL=='http://m.uzhuang.com/'){
                    $item['urls']="http://m.uzhuang.com/mobile-uzapp_article.html?uzid=".$item['id'];
                    }else if(WEBURL=='http://mdev.uz.com/'){
    ...
    //专题图
    public function thematic_photo(){
      $result_special1=$this->db->get_list('m_p_special',"`status`=1","id,cover,title,dao,updatetime,browse",0,3,$page,'intervene DESC,updatetime DESC');
      foreach ($result_special1 as $key => $value) {
        $result_special1[$key]['dao'] = strip_tags($value['dao']);
        $result_special1[$key]['updatetime'] = time_format1(strtotime($value['updatetime']));
        if(!preg_match("/[\x7f-\xff]/", $result_special1[$key]['updatetime'])){
            $result_special1[$key]['updatetime']=date('Y-m-d',strtotime($value['updatetime']));
        }
        $result_special1[$key]['cover'] = $value['cover']?"http://www.uzhuang.com/image/pic_230/".$value['cover']:'';
      }
      return $result_special1;
    }
    //推荐装修工地
    public function log($city,$status,$uid,$set_node){
      $nodes = $this->getNode();
      if($uid){
      $result= $this->db->get_list('demand',"uid='".$uid."' and orderstatus!='终止'",'id',0,1,$page,'addtime DESC');
      }
      ...
      //echo "<pre>";print_r($result);exit;
        if($res){
          $xnode = $nodes['previous'][$res[0]['nodeid']];
          if($xnode['node_id']>=1 && $xnode['node_id']<=19 || $xnode['node_id']!=101 || $xnode['node_id']!=102){
        ...
        if($where){
          $log = $this->db->get_list('day_log_demand_list',$where,'logname', 0, 1, $page, 'addtime DESC');
            if(!empty($log)){
            $log_rs = array();
            $query = $this->db->query("select * from wz_day_log_demand_list where areaid_2 is not null order by (case when ".$where." then 1 else 0 end) desc,addtime DESC limit 0,3;");
            ...
            $log_rs = array();
            $query = $this->db->query("select * from wz_day_log_demand_list where areaid_2 is not null order by (case when ".$where1." then 1 else 0 end) desc,addtime DESC limit 0,3;");
            while($data = $this->db->fetch_array($query)) {
              $log_rs[] = $data;
            }
              $log = $this->db->get_one('f_node','node_id="'.$re[0].'"','node_name', 0, 1, $page, 'addtime DESC');
        ...
        foreach ($log_rs as $key => $value) {
          ...
          if($value['renovationcategory']==1){
            ...
            $arr[$key]['log_photo']='http://m.uzhuang.com/res/msite/building_live_new/img/cover.jpg';
            }
    ...
    //广告位
    public function adsense($city){
      $where = "city like  '%,".$city.",%'";
      $res = $this->db->get_list('advertis',$where,'cover,address',0,3,$page,'intervene desc,addtime desc');
      foreach ($res as $key => $value){
        $res[$key]['cover'] = $value['cover']?"http://mpre.uz.com/image/original/".$value['cover']:'';
      }
      return $res;
    }
    //装修问答
    public function question() {
        $where="sort !=0 and yydel=0 and qtype='热门' ";
        $data = $this->db->get_list('bang_question',$where,'*',0,3,$page,'sort asc,qtime desc');
        foreach ($data as $key => $value) {
          $result = $this->db->get_list('bang_answer',array('qid'=>$value['id']),'*');
          if(empty($value['uid'])){
             $datas[$key]['id']=$value['id'];
             $datas[$key]['title']=$value['title'];
             $datas[$key]['laud']=$value['pv'];
          }else if(!empty($value['uid'])){
          $content= $this->db->get_one('member','uid='.$value['uid'],'*');
           $time= strtotime($value['qtime']);
           $datas[$key]['id']=$value['id'];
           $datas[$key]['title']=$value['title'];
           $datas[$key]['laud']=$value['pv'];
          }
    ...
    public function index(){
      ...
      $arr=array(
      //灵感图片
      'case_photo'=>$this->case_photo($city,$style,$uid,$style1),
      //装修攻略
      'getOrder'=>$this->getOrder($uid,$status,$set_node),
      //专题图
      'thematic_photo'=>$this->thematic_photo(),
      //装修问答
      'question'=>$this->question(),
      //装修工地
      'log'=>$this->log($city,$status,$uid,$set_node),
      //广告位
      'adsense'=>$this->adsense($city)
      );
      echo json_encode(array('code'=>1,'data'=>$arr,'message'=>'首页数据','process_time'=>time())); exit;
    }
