...
class password extends WUZHI_foreground
{
    ...
    {
        if (get_cookie('auth')) {
            echo json_encode(array('code' => 1, 'data' => null, 'message' => '你已登录,无需重复操作', 'process_time' => time()));
            exit;
        }
        ...
        if ($modelid == 11) {
            echo json_encode(array('code' => 0, 'data' => null, 'message' => '只有会员才可以登陆', 'process_time' => time()));
            exit;
        }
        $password = isset($GLOBALS['password']) ? $GLOBALS['password'] : '';
        if (empty($username)) {
            echo json_encode(array('code' => 0, 'data' => null, 'message' => '用户名不能为空', 'process_time' => time()));
            exit;
        }
        if (empty($password)) {
            echo json_encode(array('code' => 0, 'data' => null, 'message' => '密码不能为空', 'process_time' => time()));
            exit;
        }
        ...
        if (empty($r)) {
            echo json_encode(array('code' => 0, 'data' => null, 'message' => '用户名不存在', 'process_time' => time()));
            exit;
        }
        //	判断用户是否被锁定
        if ($r['lock']) {
            ...
            if ($r['locktime'] > SYS_TIME) {
                echo json_encode(array('code' => 0, 'data' => null, 'message' => '您的帐号已被锁定', 'process_time' => time()));
                exit;
            } else {
                //	将锁定标记改为0
        ...
        //	判断会员组是否禁止登录
        if ($r['groupid'] == 1) {
            echo json_encode(array('code' => 0, 'data' => null, 'message' => '您的帐号已被禁止访问!', 'process_time' => time()));
            exit;
        }
        //	登录记录
        ...
        if ($newpassword != $r['password']) {
            ...
            echo json_encode(array('code' => 0, 'data' => null, 'message' => '密码错误', 'process_time' => time()));
            exit;
        } else {
            $loginLog['status'] = 3;
            $this->db->insert('logintime', $loginLog);
    ...
    //2017年7月28日18:19:33 zkl
    /**
     * 短信登陆
     * @param string  手机号
     * @param string  验证码
     * @return string code 0 成功 / 1 失败
     *                message 返回消息
     *                process_time 响应时间
     */
    public function smslogin()
    {
        ...
        $savecookie = $GLOBALS['savecookie'];
        if (empty($mobile)) {
            echo json_encode(array('code' => 1, 'data' => null, 'message' => '手机号码不能为空', 'process_time' => time()));
            exit;
        }
        if (!preg_match('/^(?:13\d{9}|15[0|1|2|3|5|6|7|8|9]\d{8}|17[0|1|2|3|5|6|7|8|9]\d{8}|18[0|1|2|3|5|6|7|8|9]\d{8}|14[5|7]\d{8})$/', $mobile)) {
            echo json_encode(array('code' => 1, 'data' => null, 'message' => '手机号不正确', 'process_time' => time()));
            exit;
        }
        $s = $this->db->count('member', "mobile={$mobile}");
        if ($s['num'] <= 0) {
            echo json_encode(array('code' => 1, 'data' => null, 'message' => '手机号尚未注册', 'process_time' => time()));
            exit;
        }
        $where = "`mobile`='$mobile'";
        $r = $this->db->get_one('sms_checkcode', $where, '*', 0, 'id DESC');
        if (!$r || $r['code'] == '' || $r['code'] != $smscode) {
            echo json_encode(array('code' => 1, 'data' => null, 'message' => '短信验证码错误', 'process_time' => time()));
            exit;
        }
        if ($r['posttime'] < SYS_TIME - 300) {
            echo json_encode(array('code' => 1, 'data' => null, 'message' => '短信验证码过期，请重新发送短信！', 'process_time' => time()));
            exit;
        }
        //更新验证码数据库
        ...
        //根据手机号查出member表该用户数据
        $rs = $this->db->get_one('member',$where,'*','0','uid DESC');
        $modelid = $rs['modelid'];
        if ($modelid == 11) {
            echo json_encode(array('code' => 1, 'data' => null, 'message' => '只有会员才可以登陆', 'process_time' => time()));
            exit;
        }
        if($savecookie=='savecookie'){
            $cookietime = SYS_TIME + 3600 * 24 * 30;
        }else{
            $cookietime = SYS_TIME + 3600 * 24;
        }
        //	判断用户是否被锁定
        if ($rs['lock']) {
            //	判断是否在锁定的时间内
            if ($rs['locktime'] > SYS_TIME) {
                echo json_encode(array('code' => 1, 'data' => null, 'message' => '您的帐号已被锁定', 'process_time' => time()));
                exit;
            } else {
                //	将锁定标记改为0
                $this->db->update('member', 'lock=0', 'uid=' . $rs['uid']);
            }
        }

        //	判断会员组是否禁止登录
        if ($rs['groupid'] == 1) {
            echo json_encode(array('code' => 1, 'data' => null, 'message' => '您的帐号已被禁止访问!', 'process_time' => time()));
            exit;
        }
        //	登录记录
        $loginLog = array('uid' => $rs['uid'], 'logintime' => SYS_TIME, 'ip' => get_ip());
        $loginLog['status'] = 3;
        $this->db->insert('logintime', $loginLog);

        //	判断是否需要验证Email
        if ($this->setting['checkemail'] && $rs['groupid'] == 2) {
            if ($this->send_register_mail($rs)) {
                MSG(L('need_email_authentication'));
            } else {
                MSG(L('email_authentication_error'));
            }
        }
        $this->db->query('UPDATE `wz_member` SET `lasttime`=' . SYS_TIME . ', `lastip`="' . get_ip() . '", `loginnum`=`loginnum`+1 WHERE `uid`=' . $rs['uid'], false);
        $this->create_cookie($rs, $cookietime);
        echo json_encode(array('code' => 0, 'data' => null, 'message' => L('login_success'), 'process_time' => time()));
        exit;
    }
    ...
    public function register()
    {
        $mobile = $GLOBALS['mobile'];
        if (empty($mobile)) {
            echo json_encode(array('code' => 0, 'data' => null, 'message' => '手机号码不能为空', 'process_time' => time()));
            exit;
        }
        if (!preg_match('/^(?:13\d{9}|15[0|1|2|3|5|6|7|8|9]\d{8}|17[0|1|2|3|5|6|7|8|9]\d{8}|18[0|2|3|5|6|7|8|9]\d{8}|14[5|7]\d{8})$/', $mobile)) {
            echo json_encode(array('code' => 0, 'data' => null, 'message' => '手机号码错误', 'process_time' => time()));
            exit;
        }
        $s = $this->db->count('member', "mobile={$mobile}");
        if ($s['num'] > 0) {
            echo json_encode(array('code' => 0, 'data' => null, 'message' => '用户名已存在', 'process_time' => time()));
            exit;
        }
        $smscode = $GLOBALS['smscode'];

        if (!$smscode) {
            echo json_encode(array('code' => 0, 'data' => null, 'message' => '短信验证码错误', 'process_time' => time()));
            exit;
        }
        $where = "`mobile`='$mobile'";
        $r = $this->db->get_one('sms_checkcode', $where, '*', 0, 'id DESC');
        ...
        if ($r['posttime'] < SYS_TIME - 300) {
            echo json_encode(array('code' => 0, 'data' => null, 'message' => '短信验证码过期，请重新注册！', 'process_time' => time()));
            exit;
        }
        ...
        if ($uid) {
            ...
            if (isset($_SESSION['authid']) && $_SESSION['authid']) {
                ...
                echo json_encode(array('code' => 1, 'data' => $url, 'message' => '注册成功', 'process_time' => time()));
                exit;
            }
        }
    ...
    public function mb()
    {
        ...
        if ($s['num'] > 0) {
            ...
            echo json_encode(array('code' => 1, 'data' => null, 'message' => '用户可以注册', 'process_time' => time()));
            exit;
        }
    }
    ...
    public function logout()
    {
        ...
        echo json_encode(array('code' => 1, 'data' => null, 'message' => '退出成功', 'process_time' => time()));
        exit;
    }
    ...
    public function edit_mobile()
    {
        ...
        if ($checkcode == '') {
            echo json_encode(array('code' => 0, 'data' => null, 'message' => '请输入图片验证码！', 'process_time' => time()));
            exit;
        }

        if (strtolower($_SESSION['code']) != strtolower($checkcode)) {
            echo json_encode(array('code' => 0, 'data' => null, 'message' => '图片验证码错误！', 'process_time' => time()));
            exit;
        }
        $_SESSION['code'] = '';
        $mobile = $GLOBALS['mobile'];
        ...
        echo json_encode(array('code' => 1, 'data' => $mobile, 'message' => '验证成功', 'process_time' => time()));
        exit;
    ...
    public function edit_code()
    {
        ...
        $mobile = $GLOBALS['mobile'];
        if (!$smscode) {
            echo json_encode(array('code' => 0, 'data' => null, 'message' => '短信验证码不能为空', 'process_time' => time()));
            exit;
        }
        $where = "`mobile`='$mobile'";
        $r = $this->db->get_one('sms_checkcode', $where, '*', 0, 'id DESC');
        ...
        if ($r['posttime'] < SYS_TIME - 300) {
            echo json_encode(array('code' => 0, 'data' => null, 'message' => '短信验证码过期，请重新获取验证码！', 'process_time' => time()));
            exit;
        }

        $this->db->update('sms_checkcode', array('code' => ''), array('id' => $r['id']));
        echo json_encode(array('code' => 1, 'data' => null, 'message' => '短信验证码成功', 'process_time' => time()));
        exit;
    }
    ...
    public function edit_password()
    {
        ...
        if (empty($password)) {
            echo json_encode(array('code' => 0, 'data' => null, 'message' => '请输入密码', 'process_time' => time()));
            exit;
        }
        ...
        if ($password != $password2) {
            echo json_encode(array('code' => 0, 'data' => null, 'message' => '密码不一致', 'process_time' => time()));
            exit;
        }
        ...
        if ($this->db->query('UPDATE `wz_member` SET `password` = "' . $newpassword . '" WHERE `mobile`="' . $mobile . '"')) {
            $this->clean_cookie();
            echo json_encode(array('code' => 1, 'data' => null, 'message' => '密码修改成功', 'process_time' => time()));
            exit;
        } else {
            $this->clean_cookie();
            echo json_encode(array('code' => 0, 'data' => null, 'message' => '密码修改失败', 'process_time' => time()));
            exit;
        }
    }
}