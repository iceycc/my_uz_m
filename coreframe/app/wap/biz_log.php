...
defined('IN_WZ') or exit('No direct script access allowed');
    ...
    public function bag(){
        ...
        $arr=array();
        if($acquiesce==0){
        $log_rss = $this->db->get_list('day_log_demand_list',"addtime<'2016-07-01 11:30:16'", 'logname,userid,recphoto,addtime,nodename,orderid,nodeid,uid');
        $log_rs = $this->db->get_list('day_log_demand_list',"addtime<'2016-07-01 11:30:16'", 'logname,userid,recphoto,addtime,nodename,uid,orderid,nodeid', 0, 4, $page, 'addtime DESC');
        $c=count($log_rss);
        $page_max=ceil($c/4);
        }
        //var_dump($arr['c1']);exit;
        if($acquiesce==1) {
        $log_rsss = $this->db->get_list('day_log_demand_list',"addtime<'2016-07-01 11:30:16'", 'logname,userid,recphoto,addtime,nodename,orderid,nodeid,uid');
        $log_rs = $this->db->get_list('day_log_demand_list',"addtime<'2016-07-01 11:30:16'", 'logname,userid,recphoto,addtime,nodename,orderid,nodeid,uid', 0, 4, $page, 'browse_count DESC');
        $c=count($log_rsss);
        $page_max=ceil($c/4);
        }
        if($nodeid){
        $where = "addtime<'2016-07-01 11:30:16' and nodeid in (".$day_log[$nodeid]['nodeid'].")";
        ...
        if($ends==5){
          //echo "string";
        $where = "addtime<'2016-07-01 11:30:16' and orderstatus='完结'";
