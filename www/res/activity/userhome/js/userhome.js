...
function getTitle() {
    $.ajax({
        type: "POST",
        url: "index.php?m=zx_recommended&f=member&v=index&uid="+UID,
        dataType: "json",
        timeout: 3000,
...
function getPromote() {
    $.ajax({
        type: "POST",
        url: "index.php?m=zx_recommended&f=member&v=my_recommend&uid="+UID,
        dataType: "json",
        timeout: 3000,
        success: function(data){
            if(data.code == 1){
                ...
                $('.promotes .promote_tit a').attr('href','activity-promote.html');
            }else{
    ...
    $.ajax({
        ...
        success: function(data){
            if(data.code == 1){
                ...
                $('.round_box').width($('.round_box li.round_list').width()*($('.round_box li.round_list').size()));
            }else{
                $('.friend .round').remove();
