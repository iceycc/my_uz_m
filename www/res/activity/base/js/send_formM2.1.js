...
$(function(){
	...
	function checkpos(){
		var checkposTimer;
		clearInterval(checkposTimer);
		checkposTimer = setInterval(function(){
			var secVal = $("#select-01").val();
			if(secVal){
	...
	function alertOpen(str) {
		...
		function alertClose() {
			...
			if(t>0)
			{
				...
				//base js 尾部隐藏报名遮罩层
				if(!alertOpenbOk)return;
				document.body.scrollTop = oScrollTop;
				$(".footer_menu_bs").show();
				if($(".amount_house_mask_bs")||$(".in_info_bs")){
					//取消阻止touchmoce事件
	...
	$("#select-01").on('touchstart',function(){
		checkpos();
	});
})
