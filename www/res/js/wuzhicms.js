    ...
    top.dialog({
            ...
            if (this.returnValue) {
                if(returntype==1) {//返回缩略图＋隐藏input
                    $('#'+id+"_thumb").attr('src','http://mdev.uz.com/image/big_square/'+this.returnValue);
...
function openIframe(iframeurl,id,title,width,height,returntype) {
    top.dialog({
            ...
            if (this.returnValue) {
                if(returntype==1) {//返回缩略图＋隐藏input
                    $('.'+id+"_thumb").attr('src','http://m.uzhuang.com/image/big_square/'+this.returnValue);
                    $('.'+id).val(this.returnValue);
                }else if(returntype > 1){ //返回字符串,多文件
                    $('.'+id+" span").append(this.returnValue);
                    $('.'+id).val('1');
                }
                else {
                    $('.'+id).val(this.returnValue);
...
function linkage(fieldid,linkid,obj) {
    if(linkid!=0) {
        if($(obj).attr('id')=='LK1_2' && $(obj).attr('data-value')!=linkid) {
            linkage_box('areaids',linkid);
        }
        $("#"+fieldid).val(linkid);
    }
}
...
//for uzhuang
function linkage_box(field,pid) {
    //var pid = $("#"+pid).val();
    if(pid==null) {
        msgtip('请选择所属区域-第一项');
    } else if(pid==0) {
        ...
        $.getJSON("?m=linkage&f=json&v=linkage_box", { pid: pid},
            function(data){
                $("#"+field+"_div").html('');
                $.each(data, function( i, xa ){
                    $("#"+field+"_div").append('<label class="checkbox-inline"><input type="checkbox" name="form['+field+'][]" value="'+xa.lid+'" datatype="max20" sucmsg="OK" errormsg="请选择服务区域" nullmsg="请选择服务区域！">'+xa.name+'</label>');
                    console.log(field);
                });
            });
    }
...
function text_select(field,obj) {
    $("#"+field).val($(obj).val());
}