...
function getList() {
	...
	var reqUrl;
	/*if(nodeId == 5) {
		reqUrl = "http://mdev.uz.com/index.php?m=wap&f=sd_log&v=listl&acquiesce="+sortStatus+"&ends="+nodeId+"&page="+page;
	} else {
		reqUrl = "http://mdev.uz.com/index.php?m=wap&f=sd_log&v=listl&acquiesce="+sortStatus+"&nodeid="+nodeId+"&page="+page;
	}*/
	$('#list-container').append(liveContent);
	$.ajax({
		type: "POST",
		url: 'index.php?m=wap&f=sd_log&v=listl',
		dataType: "json",
		timeout: 2000,
