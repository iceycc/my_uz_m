...
$(function(){
    ...
    $("#gj-contact").bind('click', function(e) {
        var that = $(this);
        sendUrl(that, "#gj-name","&page_id="+gj_id+"&type=steward");
    });
    $("#gs-contact").bind('click', function(e) {
        var that = $(this);
        sendUrl(that, "#company-name","&page_id="+gs_id+"&type=works");
    });

    //确定来源
    function sendUrl(id, name,massage){
        var liveName = $("#order-name").html();
        var thisName = $(name).html();
        if(browser.versions.weixin){
                id.attr("href","mobile-ordered.html?id=微信-工地直播-"+liveName+"-"+thisName+massage);
            }else if(browser.versions.weibo){
                id.attr("href","mobile-ordered.html?id=微博-工地直播-"+liveName+"-"+thisName+massage);
            }else{
                id.attr("href","mobile-ordered.html?id=M站-工地直播-"+liveName+"-"+thisName+massage);
            }
    }
})
...
function setOrderData(data) {
    ...
    //2016-08-04详情新增s
    $(".gj_link_detio").attr("href","mobile-m_butler_details.html?id=M站-工地直播&butlerid="+data.uid);
    //2016-08-04详情新增e
